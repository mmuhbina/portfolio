...
const animatedElements = document.querySelectorAll('h1, h2, h3, p, .section-tag, .btn, .cta-button, .stat-item, .folder-card');
...
if (cursorDot && cursorOutline) {
    ...
    if (siteFooter) {
        siteFooter.addEventListener('mouseenter', () => document.body.classList.add('footer-hover'));
        siteFooter.addEventListener('mouseleave', () => document.body.classList.remove('footer-hover'));
    }
}
...
if (hamburgerBtn && hamburgerContainer) {
    ...
    // Close menu when clicking outside (Mobile Only logic)
    ...
    // Close menu when a link inside is clicked (Mobile Only logic)
    const menuLinks = hamburgerContainer.querySelectorAll('.dropdown-menu a');
